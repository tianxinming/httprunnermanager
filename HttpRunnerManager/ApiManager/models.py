...
from ApiManager.managers import UserTypeManager, UserInfoManager, ProjectInfoManager, ModuleInfoManager, \
    TestCaseInfoManager, EnvInfoManager, DebugTalkManager, TestResultManager
...
class ProjectInfo(BaseTable):
    ...
    project_simple_name = models.CharField('项目简称', max_length=50, null=False, default='default')
...
class ModuleInfo(BaseTable):
    ...
    simple_desc = models.CharField('简要描述', max_length=100, null=True)
    other_desc = models.CharField('其他信息', max_length=100, null=True)
    mode_link = models.CharField('脑图链接', max_length=200, default='', null=False)
...
class TestCaseInfo(BaseTable):
    ...
    name = models.CharField('用例/配置名称', max_length=200, null=False)
    ...
    is_complete = models.CharField('是否完成', null=False, default="Y", max_length=5)
    run_mode = models.CharField('运行模式', max_length=50, default='独立', null=False)
...
class EnvInfo(BaseTable):
    ...
    env_name = models.CharField(max_length=40, null=False)
    base_url = models.CharField(max_length=100, null=False)
    env_flag = models.CharField(max_length=40, null=True,default="test")
...
class DebugTalk(BaseTable):
    ...
    belong_project = models.ForeignKey(ProjectInfo, on_delete=models.CASCADE)
    name = models.CharField(max_length=40, null=True, default='debugtalk.py')
...
class TestResult(BaseTable):
    class Meta:
        verbose_name = "用例执行结果"
        db_table = "TestResult"

    belong_project = models.CharField('所属项目', max_length=50, null=False, unique=True, primary_key=True)

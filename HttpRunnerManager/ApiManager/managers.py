...
class ProjectInfoManager(models.Manager):
    ...
    def update_project(self, id, **kwargs):  # 如此update_time才会自动更新！！
        ...
        obj.project_simple_name = kwargs.get('project_simple_name')
...
class ModuleInfoManager(models.Manager):
    ...
    def update_module(self, id, **kwargs):
        ...
        obj.simple_desc = kwargs.get('simple_desc')
        obj.other_desc = kwargs.get('other_desc')
        obj.mode_link = kwargs.get('mode_link')
...
class TestCaseInfoManager(models.Manager):
    ...
    def insert_case(self, belong_module, **kwargs):
        ...
        print("调式:%s"%case_info)
        self.create(name=kwargs.get('test').get('name'), belong_project=case_info.pop('project'), run_mode=case_info.pop('run_mode'),
                    belong_module=belong_module,
                    author=case_info.pop('author'), include=case_info.pop('include'), is_complete=kwargs.get('test').pop("is_complete"), request=kwargs)

    def update_case(self, belong_module, **kwargs):
        case_info = kwargs.get('test')
        print("调式1:%s" % case_info)
        case_info = kwargs.get('test').pop('case_info')
        print("调式:%s" % case_info)
        ...
        obj.request = kwargs
        obj.run_mode = case_info.pop('run_mode')
        obj.is_complete = kwargs.get('test').pop('is_complete')
        print(obj.run_mode)
        obj.save()
...
class EnvInfoManager(models.Manager):
    ...
    def update_env(self, index, **kwargs):
        ...
        obj.env_flag = kwargs.pop('env_flag')
...
class DebugTalkManager(models.Manager):
    def insert_debug(self, **kwargs):
        self.create(**kwargs)

    def update_debug(self, index, **kwargs):
        # belong_project = kwargs.pop('belong_project')
        obj = self.get(id=index)
        obj.belong_project = kwargs.pop('belong_project')
        obj.name = kwargs.pop('name')
        obj.save()

    def get_debugtalk(self, index, return_type=""):
        if return_type == "project":
            return self.get(id=index).belong_project
        else:
            return self.get(id=index).debugtalk

    def del_debugtalk(self, index):
        self.get(id=index).delete()
...
class TestResultManager(models.Manager):
    ...
    def update_result(self, belong_project, **kwargs):
        obj = self.get(belong_project=belong_project)
        obj.fail_num = kwargs.pop('fail_num')
        obj.save()

    def get_result(self, belong_project):
        try:
            return self.get(belong_project=belong_project).fail_num
        except:
            return None

...
from .common import get_project_data
...
def get_pager_info(Model, filter_query, url, id, per_items=12):
    ...
    if url == '/api/project_list/':
    ...
    elif url == '/api/get_all_data/':
        from django.core.cache import cache
        obj = cache.get("project_data")
        if not obj:
            obj = get_project_data()
            cache.set("project_data", obj, timeout=600)
        ...
        if belong_project != 'All' and belong_module != '请选择':
            obj = obj.filter(belong_project=belong_project).filter(belong_module__module_name=belong_module)
        else:
            if belong_project != 'All' and belong_module == '请选择':
                obj = obj.filter(belong_project=belong_project)
        if name is not '':

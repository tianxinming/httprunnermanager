    ...
    if type:
        if project_opt.get_pro_name(project_name) < 1:
            try:
                ...
                DebugTalk.objects.create(belong_project=belong_project, debugtalk='# debugtalk.py')     # 创建debugtalk
            except DataError:
                return '项目信息过长'
            except Exception:
...
def add_case_data(type, **kwargs):
    ...
    # run_mode = kwargs.get("test").get("run_mode")
...
def env_data_logic(**kwargs):
    ...
    if index == 'add':
        ...
        try:
            if EnvInfo.objects.filter(env_name=env_name).filter(env_flag=env_flag).count() < 1:
        ...
        try:
            if EnvInfo.objects.filter(env_name=env_name).filter(env_flag=env_flag).count() > 0:
                return '环境名称和对应标识已存在'
            else:
...
def debugtalk_data_logic(**kwargs):
    """
    debugtalk逻辑判断及落地
    :param kwargs: dict
    :return: ok or tips
    ...
    if id:
        try:
            belong_project = DebugTalk.objects.get_debugtalk(id, "project")
            print("debugtalk的belong_project：", belong_project)

            if DebugTalk.objects.filter(belong_project=belong_project).count() <= 1:
                return "当前项目只有一个debugtalk，不允许删除！"
            DebugTalk.objects.del_debugtalk(id)
        except ObjectDoesNotExist:
            return '删除异常，请重试'
        return 'ok'

    index = kwargs.pop('index')
    debugtalk = kwargs.get('debugtalk')
    if debugtalk is '':
        return 'debugtalk名称不可为空'
    if not debugtalk.endswith(".py"):
        return 'debugtalk必须是.py结尾'
    ...
    if index == 'add':
        ...
        try:
            if DebugTalk.objects.filter(debugtalk=debugtalk, belong_project=belong_project).count() < 1:
                kwargs["debugtalk"] = "# "+debugtalk
                kwargs["belong_project"] = belong_project
                kwargs["name"] = debugtalk
                DebugTalk.objects.insert_debug(**kwargs)
                logging.info('debugtalk添加成功：{kwargs}'.format(kwargs=kwargs))
                return 'ok'
            else:
                return 'debugtalk名称重复'
        except DataError:
            return 'debugtalk信息过长'
        except Exception as e:
            logging.info(e)
            logging.error('添加debugtalk异常：{kwargs}'.format(kwargs=kwargs))
            return 'debugtalk添加异常，请重试'
    else:
        try:
            if DebugTalk.objects.get_debugtalk(index) != debugtalk and DebugTalk.objects.filter(
                    debugtalk=debugtalk, belong_project=belong_project).count() > 0:
                return 'debugtalk名称已存在'
            else:
                kwargs["belong_project"] = belong_project
                kwargs["name"] = debugtalk
                DebugTalk.objects.update_debug(index, **kwargs)
                logging.info('debugtalk更新成功：{kwargs}'.format(kwargs=kwargs))
                return 'ok'
        except DataError:
            return 'debugtalk名称过长'
        except ObjectDoesNotExist:
            logging.error('debugtalk信息查询失败：{kwargs}'.format(kwargs=kwargs))
            return '更新失败，请重试'

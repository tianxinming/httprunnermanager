...
from ApiManager.models import ModuleInfo, TestCaseInfo, TestReports, TestSuite, ProjectInfo, TestResult
...
def project_info_logic(type=True, **kwargs):
    ...
    if kwargs.get('project_name') is '':
        return '项目名称不能为空'
    if kwargs.get('project_simple_name') is '':
        return '项目简称不能为空'
...
def case_info_logic(type=True, **kwargs):
    ...
    if 'request' not in test.keys():
        ...
        if url and validate:
            test.setdefault("is_complete", "Y")
        else:
            test.setdefault("is_complete", "N")
...
def timestamp_to_datetime(summary, type=True):
    ...
    for detail in summary['details']:
        try:
            ...
            pass
        print("detail['records']：", detail['records'])
...
def get_case_count(project):
    """
    获取指定项目下已完成、未完成、总共的用例数
    :param kwargs:
    :return:
    """
    try:
        case_count_complete = TestCaseInfo.objects.filter(belong_project=project, is_complete="Y", type=1).count()
        case_count_not_complete = TestCaseInfo.objects.filter(belong_project=project, is_complete="N", type=1).count()
        case_count_all = case_count_complete + case_count_not_complete
    except Exception as e:
        case_count_complete = 0
        case_count_not_complete = 0
        case_count_all = 0
        print(e)

    result = {"project": project, "all": case_count_all, "complete": case_count_complete, "not_complete": case_count_not_complete}
    return result


def get_top_case_count():
    """获取setting中配置的前4个项目的统计数据"""
    projects = settings.TOP_PROJECTS
    results = []
    for project in projects[0:4]:
        ...
        case_count_not_complete = TestCaseInfo.objects.filter(belong_project=project, is_complete="N", type=1).count()
        case_count_all = case_count_complete + case_count_not_complete
        results.append({"project": project, "all": case_count_all, "complete": case_count_complete, "not_complete": case_count_not_complete})
...
def get_project_data():
    """
    统计各项目总用例数、已完成用例数、已完成用例占比、成功率、未完成用例数、未完成用例占比
    :return:
    """
    projects = ProjectInfo.objects.all().values("project_name").order_by('-update_time')
    project_info_list = []
    for project in projects:
        try:
            case_count_complete = TestCaseInfo.objects.filter(belong_project=project["project_name"], is_complete="Y", type=1).count()
            case_count_not_complete = TestCaseInfo.objects.filter(belong_project=project["project_name"], is_complete="N", type=1).count()
            case_count_all = case_count_complete + case_count_not_complete

            result = TestResult.objects.filter(belong_project=project["project_name"]).first()
            print(result)
            if result:
                success_num = case_count_complete - result.fail_num        # 最新执行用例成功的数量，用已完成的用例数减去失败的用例数
            else:
                success_num = 0
            if case_count_all != 0:
                complete_per = "0%" if case_count_complete == 0 else str(round(100 * case_count_complete / float(case_count_all), 2)) + "%"
                not_complete_per = "0%" if case_count_not_complete == 0 else str(round(100 * case_count_not_complete / float(case_count_all), 2)) + "%"
                success_per = "0%" if (success_num == 0 or success_num is None) else str(round(100 * success_num / float(case_count_all), 2)) + "%"
            else:
                complete_per = "0%"
                not_complete_per = "0%"
                success_per = "0%"
            ...
            success_per = "0%"
            print(e)
        project_info = {"project": project["project_name"], "all": case_count_all, "complete": case_count_complete,
                        "complete_per": complete_per, "success_per": success_per, "not_complete": case_count_not_complete, "not_complete_per": not_complete_per}
